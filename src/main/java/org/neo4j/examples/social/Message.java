...
public class Message
{
    ...
    public long save(Transaction tx) throws SocialNetworkException
    {
        ...
        if (inReplyTo == null)
        {
            result = tx.run(CREATE_MESSAGE, parameters("email", author.email(), "uuid", uuid.toString(), "text", text));
        }
        else {
            result = tx.run(CREATE_REPLY, parameters("email", author.email(), "uuid", uuid.toString(), "text", text, "related_uuid", inReplyTo.uuid.toString()));
        }
        if (result.hasNext())
        {
            tx.success();
        ...
        {
            throw new SocialNetworkException("Author not saved");
        }
    }
